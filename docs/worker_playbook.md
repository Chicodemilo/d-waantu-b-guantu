...
**What goes in `memory.md`:** durable lessons only ("next time you migrate enums in MySQL, autogenerate misses them; hand-write"), plus in-flight notes that are on their way to becoming one ("trying X, hit Y, working around with Z"). Not session wrap-ups, not what you shipped. Because the file is capped, prefer condensing old blocks over hoarding; future-you reads the distilled version, not the raw log.
